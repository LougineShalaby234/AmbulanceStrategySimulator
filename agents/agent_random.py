...
"""
Custom agent that:
1) Iterates over emergencies (past then recent) to assign available ambulances.
2) For each ambulance that has arrived at an emergency and has the patient, assigns a hospital based on free bed availability.
3) With a small probability, relocates idle ambulances to a random hospital or rescue station.
"""
