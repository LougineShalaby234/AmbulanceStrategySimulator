...
assert all(0 <= p <= 1 for p in [PROB_NO_TRANSPORT, PROB_DELAY]), "Probabilities must be between 0 and 1"
onsite_time_max = 10 # maximum time the ambulance remain at Emergency scene 
cleaning_time_max = 5 # maximum time the ambulance takes to clean after Hospital transport
AVG_SPEED_KPH = 40 # average speed the vehichle would move , used in estimate_travel_time
# -------------
# Entity Classes
# -------------

class Ambulance:
    def __init__(self, amb_id, lat, lng, status=AMBULANCE_STATUS_IDLE):
        self.id = amb_id
...
class Hospital:
    def __init__(self, hosp_id, lat, lng, num_beds, free_beds):
        self.id = hosp_id
        self.location = (lat, lng)
...
class RescueStation:
    def __init__(self, station_id, lat, lng, available_ambulances):
        self.id = station_id
        self.location = (lat, lng)
...
def read_map(path):
    ...
    for rs_data in data.get("rescueStations", []):
        ...
        station_obj = RescueStation(station_id, lat, lng, 0)
    ...
    for amb_data in data.get("ambulances", []):
        ...
        if reported_status == "outOfService":
            ...
            status = AMBULANCE_STATUS_IDLE
        ambulance_obj = Ambulance(amb_id, lat, lng, status)
...
def estimate_travel_time(distance_km, avg_speed_kph=AVG_SPEED_KPH):
    """
    ...
    for ambulance in state.ambulances:
        if ambulance.status == AMBULANCE_STATUS_AT_EMERGENCY and ambulance.onsite_time_remaining ==1 :
            ...
            if em and not em.resolved and not em.transport_decision_made:
                ...
                if random.random() < PROB_NO_TRANSPORT:
        ...
        if ambulance.remaining_route:
            ...
            if reached_end:
                if ambulance.status == AMBULANCE_STATUS_EN_ROUTE_EMERGENCY:
                    ...
                    ambulance.onsite_time_remaining = random.randint(2, onsite_time_max)
                    event_log.write(
                    ...
                    if hospital and emergency:
                        ...
                        ambulance.cleaning_time_remaining = random.randint(2, cleaning_time_max)
                        event_log.write(
...
def compute_route(start_point, end_point, api=ROUTING_METHOD):
    """assert ROUTING_METHOD in ["Google", "Here", "Openrouteservice", "LinearInterpolation", "Valhalla"] .....
